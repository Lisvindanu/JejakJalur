...
import type { BookmarkProfil, SharedProps, UlasanProfil } from '@/types';
...
function kategoriPlaceholder(kategori: string) {
    if (kategori === 'Kuliner') {
        return { gradient: 'from-amber-100 to-amber-200', icon: <IconToolsKitchen2 size={28} className="text-black/20" /> };
    }
    if (kategori === 'UMKM') {
        return { gradient: 'from-indigo-100 to-indigo-200', icon: <IconShoppingBag size={28} className="text-black/20" /> };
    }
    return { gradient: 'from-emerald-100 to-emerald-200', icon: <IconMountain size={28} className="text-black/20" /> };
}
    ...
    return (
        <div className="group flex flex-col overflow-hidden rounded-xl border border-stone-100 bg-white transition-shadow hover:shadow-md">
            <Link href={`/destinasi/${destinasi.id}`} className="no-underline">
                <div className={`flex h-32 items-center justify-center bg-gradient-to-br ${ph.gradient}`}>
                    {destinasi.foto_url ? (
                        <img src={destinasi.foto_url} alt={destinasi.nama} className="h-full w-full object-cover" />
                    ) : (
                        ph.icon
                    )}
                </div>
...
function UlasanCard({ ulasan }: { ulasan: UlasanProfil }) {
    ...
    return (
        <div className="flex gap-4 rounded-xl border border-stone-100 bg-white p-4">
            <Link href={`/destinasi/${ulasan.destinasi.id}`} className="no-underline shrink-0">
                <div className={`flex h-14 w-14 items-center justify-center overflow-hidden rounded-lg bg-gradient-to-br ${ph.gradient}`}>
                    {ulasan.destinasi.foto_url ? (
                        <img src={ulasan.destinasi.foto_url} alt={ulasan.destinasi.nama} className="h-full w-full object-cover" />
                    ) : (
                        ph.icon
                    )}
                </div>
            ...
            <div className="min-w-0 flex-1">
                <div className="flex flex-wrap items-start justify-between gap-1">
                    ...
                    <span className="text-xs text-stone-400">{formatTanggal(ulasan.created_at)}</span>
                </div>
                <RatingDisplay value={ulasan.rating} size={13} className="mt-1" />
                {ulasan.judul && (
                    <p className="mt-1 text-xs font-medium text-stone-700">{ulasan.judul}</p>
                )}
                <p className="mt-0.5 line-clamp-2 text-xs text-stone-500">{ulasan.konten}</p>
            </div>
        </div>
    );
}
                ...
                <div className="mx-auto max-w-4xl space-y-6">
                    ...
                    <div className="rounded-2xl border border-stone-200 bg-white p-6 sm:p-8">
                        <div className="mb-4 flex items-center gap-2">
                            <IconBookmark size={18} className="text-amber-600" />
                            <h2 className="font-semibold text-stone-800">Wishlist Destinasi</h2>
                            {bookmarks.length > 0 && (
                        ...
                        {bookmarks.length === 0 ? (
                            <div className="rounded-xl border border-dashed border-stone-200 py-10 text-center">
                                <IconBookmark size={32} className="mx-auto mb-2 text-stone-300" />
                                <p className="text-sm text-stone-400">Belum ada destinasi yang disimpan.</p>
                                <Link
                                    href="/destinasi"
                    ...
                    <div className="rounded-2xl border border-stone-200 bg-white p-6 sm:p-8">
                        <div className="mb-4 flex items-center gap-2">
                            <IconPencil size={18} className="text-emerald-700" />
                            <h2 className="font-semibold text-stone-800">Riwayat Ulasan</h2>
                            {ulasan.length > 0 && (
                                <span className="ml-auto rounded-full bg-stone-100 px-2 py-0.5 text-xs text-stone-500">
                                    {ulasan.length}
                                </span>
                        ...
                        {ulasan.length === 0 ? (
                            <div className="rounded-xl border border-dashed border-stone-200 py-10 text-center">
                                <IconPencil size={32} className="mx-auto mb-2 text-stone-300" />
                                <p className="text-sm text-stone-400">Belum ada ulasan yang ditulis.</p>
                                <Link
                                    href="/destinasi"
                                    className="mt-2 inline-block text-sm font-medium text-emerald-700 no-underline hover:text-emerald-800"
                                >
